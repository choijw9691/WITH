...
class App : Application() {
    ...
    private class KakaoSDKAdapter : KakaoAdapter() {
        ...
        override fun getApplicationConfig(): IApplicationConfig {
            return IApplicationConfig { lets.go.lets_go_together.App.Companion.globalApplicationContext }
        }
    }

    override fun onCreate() {
        super.onCreate()
        lets.go.lets_go_together.App.Companion.instance = this

        KakaoSDK.init(lets.go.lets_go_together.App.KakaoSDKAdapter())
    }

    override fun onTerminate() {
        super.onTerminate()
        lets.go.lets_go_together.App.Companion.instance = null
    }

    companion object {

        @Volatile
        private var instance: lets.go.lets_go_together.App? = null

        val globalApplicationContext: lets.go.lets_go_together.App?
            get() {
                checkNotNull(lets.go.lets_go_together.App.Companion.instance) { "this application does not inherit com.kakao.GlobalApplication" }
                return lets.go.lets_go_together.App.Companion.instance
            }
    }
}
